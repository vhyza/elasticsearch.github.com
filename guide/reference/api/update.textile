...
p. There is also support for @upsert@ (since 0.20).  If the document does not already exists, the content of the @upsert@ element will be used to index the fresh doc:
...
p. Last it also supports @doc_as_upsert@ (since 0.90.2). So that the provided document will be inserted if the document does not already exist. This will reduce the amount of data that needs to be sent to elasticsearch.
