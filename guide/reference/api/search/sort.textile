...
p. Be careful when using this parameter with cross-indices search requests. You may encounter failures or unconsistent results if the unmapped fields are not the same on all indices which are being queried.
