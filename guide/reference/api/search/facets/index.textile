...
In the @terms@ array, relevant _terms_ and _counts_ are returned. You'll probably want to display these to your users. The facet also returns the number of documents which have no value for the field (@missing@), the number of documents for facet values not included in the returned facets (@other@), and the total number of documents for all tokens in the facet (@total@).
