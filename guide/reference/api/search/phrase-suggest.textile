---
layout: guide
title: Search API - Phrase Suggest
cat: guide
...
p. The response contains suggested scored by the most likely spell correction first. In this case we got the expected correction @xorr the god jewel@ first while the second correction is less conservative where only one of the errors is corrected. Note, the request is executed with @max_errors@ set to @0.5@ so 50% of the terms can contain misspellings (See parameter descriptions below).
