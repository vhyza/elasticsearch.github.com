...
p. Add put index alias api for adding a single index alias. Options:
...
p. Add delete index alias api for deleting a single alias. Options:
...
p. There is also head veriant of the get indices aliases api to check if index aliases exist. The indices aliases exists api supports the same option as the get indices aliases api. Examples:
