...
p. Automatic index creation can include a pattern based white/black list, for example, set @action.auto_create_index@ to @+aaa*,-bbb*,+ccc*,-*@ (+ meaning allowed, and - meaning disallowed). Note, this feature is available since 0.20.
...
p. The index operation is directed to the primary shard based on its route (see the Routing section above) and performed on the actual node containing this shard. After the primary shard completes the operation, if needed, the update is distributed to applicable replicas.
