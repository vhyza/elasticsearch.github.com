...
p. The store module allows you to control how index data is stored.
...
p. The @simplefs@ type is a straightforward implementation of file system storage (maps to Lucene @SimpleFsDirectory@) using a random access file. This implementation has poor concurrent performance (multiple threads will bottleneck). It is usually better to use the @niofs@ when you need index persistence.
