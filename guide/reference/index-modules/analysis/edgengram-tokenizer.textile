...
There used to be a @side@ parameter up to @0.90.1@ but it is now deprecated. In order emulate the behavior of @"side" : "BACK"@ a @reverse@ filter should be used together with @edgeNGram@ token filter. The @edgeNGram@ filter must be enclosed in @reverse@ filters like this: @"filter" : ["reverse", "edgeNGram", "reverse"]@ which essentially reverses the token, builds front EdgeNGrams and reverses the ngram again. This has the same effect as the previous @"side" : "BACK"@ setting.
